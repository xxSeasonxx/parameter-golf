...
- **Tied embed_lr=0.05 is optimal (KILLED LR sweep upward)**: exp_060 embed_lr=0.1 (2x default) gives +0.030 BPB — the largest regression from any LR sweep. Tied embeddings serve dual duty (input lookup + output projection), making them extremely sensitive to LR. Higher LR destabilizes the shared representation. Early training shows explosive loss (step 2: loss=24.4, step 3: loss=19.9 vs typical ~6.9). The warmup phase partially recovers but the damage to early feature learning persists. Don't sweep embed LR upward.
