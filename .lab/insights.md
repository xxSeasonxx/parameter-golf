...
- **GELU² is worse than LeakyReLU(0.5)² [KNOWN, KILLED]**: exp_068 val_bpb=1.6373 (+0.0158 BPB vs best). GELU's Gaussian gating kills negative inputs exponentially (zero gradient for x < -2), while LeakyReLU(0.5) preserves 50% linearly. With squaring already providing sparsity, the activation's role is to control negative gradient flow — more flow is better. Ordering: LeakyReLU(0.5)² > GELU² > ReLU² by negative gradient preservation. Smaller artifact (12.7MB) confirms more compressible but worse quality. Activation exploration is CLOSED: LeakyReLU(0.5)² is optimal.
