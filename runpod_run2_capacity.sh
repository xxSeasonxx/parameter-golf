#!/bin/bash
# =============================================================================
# Run 2: COMPRESSION FOR CAPACITY
# =============================================================================
# Question: Does bigger model + zstd-compressed int8 beat the 11-layer baseline?
# Config:   13 layers, int8, zstd — uses compression headroom for more depth
# Expected: Larger int8+zstd artifact than Run 1. If capacity wins → push this path
# =============================================================================
set -euo pipefail

export RUN_ID=h100_run2_capacity
export SEED=1337

# Architecture — 13 layers tests capacity under active int8+zstd serialization
export NUM_LAYERS=13
export MLP_MULT_ASYMMETRIC=2,4
export FREQ_SKIP_GATING=1

# Optimizer
export MUON_WEIGHT_DECAY=0.10
export GRAD_CLIP_NORM=0.5
export LAYER_LR_SCALE=0.5
export WARMUP_STEPS=50

# QAT (pre-warmdown, targeting active int8 quantization)
export QAT_PREWARMDOWN=1
export QAT_STRENGTH=0.1
export QAT_STOP_LR_MUL=0.8
export QAT_EVERY=10

# Training
export TRAIN_BATCH_TOKENS=524288
export ITERATIONS=20000
export VAL_LOSS_EVERY=500
export MAX_WALLCLOCK_SECONDS=600

# Serialization — zstd for maximum compression
export USE_ZSTD=1
export ZSTD_LEVEL=22
export INT8_KEEP_FLOAT_FP16_NAME_PATTERNS=tok_emb

torchrun --nproc_per_node=8 train_gpt.py 2>&1 | tee run2_capacity.log
